...
async def hiro(n_requests=1000, concurrency=10, output_folder="./results/async",
               prometheus_url="http://your_prometheus_server:9090",wait_time=20,service_urls=""):
    prometheusClient = PrometheusClient(prometheus_url)
    start_time= prometheusClient.get_current_time()-20
    ...
    print(f"等待{wait_time}秒，觀察硬體資源")
    time.sleep(wait_time)
    end_time = prometheusClient.get_current_time()
    ...
    #endregion
    #region cpu_util
    cpu_util_query = '100 - (rate(node_cpu_seconds_total{instance="172.22.9.141:9100", mode="idle"}[1m]) * 100)'
    cpu_util_ts = prometheusClient.query_range(cpu_util_query, start_time, end_time, 1)
    print(f"cpu_util_ts:  {cpu_util_ts}")
    if cpu_util_ts:
        timestamps, values = zip(*cpu_util_ts)
        values = list(map(float, values))
        time_labels = [now_tw.fromtimestamp(float(ts)) for ts in timestamps]
...
async def hiro_pro(execute_time=100, concurrency=10, output_folder="./results/async",
               prometheus_url="http://your_prometheus_server:9090",wait_time=20,service_urls=""):
    prometheusClient = PrometheusClient(prometheus_url)
    start_time= prometheusClient.get_current_time()
    ...
    step="1"
    gpu_util_query='DCGM_FI_DEV_GPU_UTIL{job="prometheus/dcgm-exporter", instance="10.244.2.108:9400", gpu="0"}'
    gpu_util_ts = prometheusClient.query_range( gpu_util_query, 1744628272, 1744628327, step)
    print(f"Prometheus GPU Utilization Time Series:{gpu_util_ts}")
    if gpu_util_ts:
        timestamps, values = zip(*gpu_util_ts)
        values = list(map(float, values))
...
if __name__ == '__main__':
    ...
    service_urls=service_set[0:2]
    num=len(service_urls)
    filename = now_tw.strftime("%m%d_%H:%M")
    
    asyncio.run(hiro(n_requests=1000, concurrency=5, output_folder=f"./results/multi/test/{num}/{filename}/async/pose",
                      prometheus_url="http://172.22.9.249:30000",wait_time=5,service_urls=service_urls))
    """ asyncio.run(hiro_duration(execute_time=45, concurrency=5, output_folder=f"./results/multi/{num}/async/pose",
                      prometheus_url="http://172.22.9.249:30000",wait_time=50,service_urls=service_urls)) """
    """ asyncio.run(main(n_requests=10, concurrency=10, output_folder="./results/async/pose",
                      prometheus_url="http://172.22.9.249:30000",wait_time=20)) """

    """ asyncio.run(hiro_pro(execute_time=5, concurrency=5, output_folder=f"./results/multi/{num}/async/pose",
        prometheus_url="http://172.22.9.249:30000",wait_time=50,service_urls=service_urls)) """
